...
import { WisataItem, HomepageData } from "@/lib/admin/types";
...
export default function AdminWisataBudayaPage() {
  ...
  const loadData = async () => {
    const [data, hp] = await Promise.all([getWisataList(), getHomepageData()]);
    setList(data);
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        >
          <span>+ Tambah Wisata / Tradisi</span>
        </button>
      </div>
        ...
        <div className="bg-white rounded-3xl border border-gray-200/70 p-6 shadow-ios space-y-4">
          ...
          <div>
            <div className="flex flex-col sm:flex-row gap-3 items-center">
                ...
                onChange={async (e) => {
                  ...
                  if (file) {
                    try {
                      ...
                      setHomepage({ ...homepage, heroImageWisata: url });
                    } catch (err: unknown) {
                      console.error(err);
              ...
              <input
                type="text"
                value={homepage.heroImageWisata || ""}
                onChange={(e) => setHomepage({ ...homepage, heroImageWisata: e.target.value })}
            ...
            {homepage.heroImageWisata && (
              <div className="mt-3 relative w-full h-32 rounded-xl overflow-hidden border border-gray-200 shadow-sm">
                <img src={homepage.heroImageWisata} alt="Preview Hero" className="w-full h-full object-cover" />
              </div>
            )}
          </div>
        </div>
